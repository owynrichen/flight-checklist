```{=latex}
% data-category: green
```
# \emoji{✅} PA‑32‑260 CHEROKEE SIX

```{=latex}
% data-category: green
```
## FLOW + VERIFY CHECKLIST (POH‑ALIGNED)

**Aircraft:** PA‑32‑260 Cherokee Six

**Tail Number:** **N563VW**

**Engine:** **Lycoming O‑540‑E4B5**

```{=latex}
% data-category: blue
```
## \emoji{📘} IFR PERFORMANCE PROFILES

| Phase            | Power      | Pitch     | Config | Speed   | V/S      |
|------------------|------------|-----------|--------|---------|----------|
| Takeoff          | FULL       | +7°       | TO     | 65 MPH  | —        |
| Climb            | CLIMB SET  | +3°       | CLEAN  | Vy 105  | +800 fpm |
| Cruise           | ~15 in MP  | 0°        | Clean  | —       | —        |
| Descent          | Low / Idle | −2 to −3° | Clean  | 138 MPH | −500 fpm |
| Approach Level 1 | ~15 in MP  | 0 to −1°  | 25°    | 104 MPH | −500     |
| Approach Level 2 | ~17 in MP  | 0°        | 33°    | 104 MPH | −800     |
| Missed           | FULL       | +7°       | Clean  | 97 MPH  | +800     |

```{=latex}
% data-category: green
```
# \emoji{🟩} NORMAL OPERATIONS

```{=latex}
% data-category: grey
```
## \emoji{🧭} PREFLIGHT / TAXI

*Style: Heading 2 (Green icon prefix)*

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Fuel Quantity — CHECK / PROPER TANK

\emoji{☐} Oil — CHECK

\emoji{☐} Controls — FREE / CORRECT

\emoji{☐} Flight Instruments — SET / NO FLAGS

\emoji{☐} Documents / Charts — ON BOARD / CURRENT

\emoji{☐} Garmin 530W Database — CURRENT / IFR LEGAL

\emoji{☐} Garmin 530W Flight Plan — LOADED / VERIFIED

\emoji{☐} Trim — SET TAKEOFF

\emoji{☐} Brakes — CHECK

\emoji{☐} Avionics — AS REQUIRED

\emoji{☐} Taxi — CLEAR / CONTROLLED

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Attitude Indicator — ERECT / NO TUMBLE

\emoji{✔} Heading Indicator — ALIGNED / SET

\emoji{✔} Turn Coordinator — ALIVE / RESPONDS

\emoji{✔} Altimeter — SET / CROSS-CHECKED

\emoji{✔} CDI / OBS — IDENT / SET

\emoji{✔} Suction — IN GREEN / STABLE

\emoji{✔} Transponder — SET / ALT MODE

\emoji{✔} COM / NAV Radios — SET / IDENTIFIED

```{=latex}
% data-category: grey
```
## \emoji{\emoji{🔍}} RUN‑UP / BEFORE TAKEOFF

*Style: Heading 2*

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Parking Brake — SET

\emoji{☐} Throttle — 2000 RPM

\emoji{☐} Magnetos — CHECK (within limits)

\emoji{☐} Carburetor Heat — CHECK → OFF

\emoji{☐} Engine Gauges — GREEN

\emoji{☐} Suction — CHECK

\emoji{☐} Throttle — IDLE / SMOOTH

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Stable RPM

\emoji{✔} No abnormal vibration

\emoji{✔} Engine indications normal

```{=latex}
% data-category: blue
```
## \emoji{🛫} TAKEOFF / CLIMB

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Fuel Pump — ON

\emoji{☐} Lights — AS REQUIRED

\emoji{☐} Flaps — SET

\emoji{☐} Power — SET TAKEOFF POWER

\emoji{☐} Engine Gauges — GREEN / FULL POWER

\emoji{☐} Airspeed Alive — CHECK

\emoji{☐} Rotate — **65 MPH**

\emoji{☐} Climb Power — SET / VERIFY

\emoji{☐} Climb — **Vy 105 MPH**

```{=latex}
% data-category: blue
```
# \emoji{\emoji{🔵}} INSTRUMENT FLIGHT RULES (IFR)

```{=latex}
% data-category: blue
```
## \emoji{✈}️ IFR — DEPARTURE / ENROUTE

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Clearance — REVIEW (route / altitude / frequencies)

\emoji{☐} Bugs — HDG / ALT / MINs SET

\emoji{☐} Navigation Source — CONFIRM

\emoji{☐} Instruments — CHECK (gyro / CDI sensing)

\emoji{☐} Fuel Pump — ON

\emoji{☐} Lights — AS REQUIRED

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Garmin 530W Database — CURRENT / APPROVED

\emoji{✔} Garmin 530W Flight Plan — ACTIVE / CROSS-CHECKED

\emoji{✔} CDI Source — GPS / VLOC AS REQUIRED

\emoji{✔} Approach Frequency — SET / MONITOR

```{=latex}
% data-category: blue
```
### \emoji{📘} GARMIN 530W IFR QUICK CONFIG

\emoji{☐} Power Up — SELF TEST / NO FLAGS

\emoji{☐} Database Page — CHECK / CURRENT

\emoji{☐} Flight Plan — LOAD / ACTIVATE

\emoji{☐} CDI Source — GPS OR VLOC / VERIFY

\emoji{☐} OBS Mode — SET / RESET AS REQUIRED

\emoji{☐} Approach — SELECT / BRIEF

\emoji{☐} Comms / Nav — CROSS-CHECK / IDENT

\emoji{☐} Procedures — LOAD / VERIFY

```{=latex}
% data-category: blue
```
## \emoji{🎯} IFR — APPROACH CONFIGURATION

```{=latex}
% data-category: green
```
### \emoji{▶} IN-RANGE

\emoji{☐} Plate Brief — DSCRAM

\emoji{☐} Communications — SET / MONITOR

\emoji{☐} Missed Approach — REVIEW / BRIEF

\emoji{☐} 6 Ts — CYCLE / CONFIRM

\emoji{☐} DADS — BRIEF / CONFIRM

```{=latex}
% data-category: green
```
### \emoji{▶} PRE-FAF

\emoji{☐} BCGUMPFS — BRIEF / CONFIRM

\emoji{☐} Course Interception — SET / VERIFY

\emoji{☐} Course Alive — CALL / VERIFY

\emoji{☐} Glideslope Alive — CALL / VERIFY

\emoji{☐} Groundspeed — VERIFY / TREND

\emoji{☐} 6 Ts — CHECK / CONFIRM

\emoji{☐} DADS — CHECK / CONFIRM

```{=latex}
% data-category: green
```
### \emoji{▶} FAF

\emoji{☐} Power — SET

\emoji{☐} Pitch — SET

\emoji{☐} Configuration — FLAPS \_\_\_°

\emoji{☐} Airspeed — \_\_\_ MPH

\emoji{☐} Sink Rate — \_\_\_ FPM

\emoji{☐} DA / DH / MDA — CALL OUT

\emoji{☐} DADS — EVERY 500 FT

\emoji{☐} Missed Approach — CALL OUT / EXECUTE AS REQUIRED

\emoji{✅} *Goal: Stabilized by FAF*

```{=latex}
% data-category: emergency
```
## \emoji{🚨} MISSED APPROACH (MEMORY ITEMS)

\emoji{⛔} **IMMEDIATE ACTIONS**

- **Power** — FULL

- **Pitch** — +7°

- **Configuration** — CLEAN

- **Airspeed** — 97 MPH

- **Climb** — +800 fpm

```{=latex}
% data-category: blue
```
## \emoji{🛬} LANDING / GO‑AROUND

```{=latex}
% data-category: blue
```
### Landing — FLOW

\emoji{☐} Airspeed — AS REQUIRED

\emoji{☐} Flaps — AS REQUIRED

```{=latex}
% data-category: green
```
### Go‑Around — FLOW

\emoji{⛔} Power — FULL

\emoji{☐} Carb Heat — OFF

\emoji{☐} Flaps — RETRACT IN STAGES

\emoji{☐} Airspeed — ≥95 MPH

```{=latex}
% data-category: emergency
```
# \emoji{🟥} EMERGENCY & ABNORMAL OPERATIONS

```{=latex}
% data-category: emergency
```
## \emoji{🚨} CRITICAL MEMORY ITEMS

*(Always accessible)*

\emoji{⛔} Engine Failure — **100 MPH**

\emoji{⛔} Engine Fire (In‑Flight) — **Mixture IDLE CUTOFF**

\emoji{⛔} Electrical Fire / Smoke — **MASTER OFF**

```{=latex}
% data-category: emergency
```
## \emoji{🔥} ENGINE FAILURE — IN FLIGHT

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{⛔} Airspeed — **100 MPH (BEST GLIDE)**

\emoji{☐} Fuel Selector — FULLEST TANK

\emoji{☐} Fuel Pump — ON

\emoji{☐} Mixture — FULL RICH

\emoji{☐} Carb Heat — ON

\emoji{☐} Throttle — OPEN

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Power restored → Resume normal operations

```{=latex}
% data-category: blue
```
## \emoji{🔁} ENGINE RESTART — IN FLIGHT (POH)

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Fuel Selector — FULLEST

\emoji{☐} Fuel Pump — ON

\emoji{☐} Mixture — FULL RICH

\emoji{☐} Carb Heat — ON

\emoji{☐} Throttle — 1″ OPEN

\emoji{☐} Magnetos — BOTH

\emoji{☐} Primer — AS REQUIRED

\emoji{☐} Engine Gauges — CHECK

```{=latex}
% data-category: blue
```
## \emoji{🟫} FORCED LANDING

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{⛔} Airspeed — **100 MPH**

\emoji{☐} Landing Site — SELECT

\emoji{☐} Mixture — IDLE CUTOFF

\emoji{☐} Fuel Selector — OFF

\emoji{☐} Fuel Pump — OFF

\emoji{☐} Magnetos — OFF

\emoji{☐} Master — OFF

\emoji{☐} Seatbelts — SECURE

\emoji{☐} Door — UNLATCH

\emoji{☐} Flaps — AS REQUIRED

```{=latex}
% data-category: emergency
```
## \emoji{🔥} ENGINE FIRE — IN FLIGHT

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{⛔} Mixture — IDLE CUTOFF

\emoji{☐} Fuel Selector — OFF

\emoji{☐} Fuel Pump — OFF

\emoji{☐} Magnetos — OFF

\emoji{☐} Cabin Heat / Air — OFF

\emoji{⛔} Airspeed — **125 MPH (TOP WHITE ARC)**

➡ Proceed to **Engine Failure / Forced Landing**

```{=latex}
% data-category: emergency
```
## \emoji{⚡} ELECTRICAL FIRE / SMOKE (POH‑ALIGNED)

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{⛔} Master Switch — OFF

\emoji{☐} All Electrical Switches — OFF

\emoji{☐} Cabin Heat — OFF

\emoji{☐} Air / Vents — OPEN (ventilate)

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Smoke dissipating

```{=latex}
% data-category: green
```
### \emoji{🔄} IF ELECTRICAL POWER REQUIRED

\emoji{☐} Master — ON

\emoji{☐} Essential Circuits — ON **ONE AT A TIME**

\emoji{⛔} If smoke returns:

➡ Master OFF → **Land ASAP**

```{=latex}
% data-category: green
```
## \emoji{⚠️} ALTERNATOR FAILURE / NO CHARGE

```{=latex}
% data-category: green
```
### \emoji{▶} FLOW

\emoji{☐} Electrical Load — REDUCE

\emoji{☐} Alternator Field CB — CHECK / RESET

\emoji{☐} Alternator Output CB — CHECK / RESET

\emoji{☐} Alternator — OFF (30 sec) → ON

```{=latex}
% data-category: green
```
### \emoji{✔} VERIFY

\emoji{✔} Ammeter shows system load

\emoji{⛔} If no output:

➡ Alternator OFF → Minimal load → **Land ASAP / Practical**
